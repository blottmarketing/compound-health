...
- **Rules.**
  ...
  6. **Two section patterns, never the same one twice in a row.** `.bento` is a six-column grid of filled cells of unequal size, one of them holding a white product card (used by "what you get"). `.prod` is the interactive two-column pattern: a `role="tablist"` of choices on the left driving a card in a soft panel on the right (used by "how we choose"). Every label inside a panel or card is lifted verbatim from copy already in that section: a panel illustrates copy, it never introduces new copy.
