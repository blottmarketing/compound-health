...
Five changes are made in the port and no others:
  ...
  7. The client's revisions in scripts/client_revisions.py are applied on top.
     Each one asserts its target, so a drop that changes the wording underneath
     stops the port rather than silently losing the revision. Read that file's
     docstring: it also records the one revision deliberately NOT applied.
  8. Launch scaffolding the drop has no notion of is added to the layout: the
...
'''
    ...
    if client_revisions.APPLIED:
        print(f'\n  {len(client_revisions.APPLIED)} client revision(s) applied:')
